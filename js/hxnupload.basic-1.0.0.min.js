var ZYFILE = {
	...
	funUploadFile : function(file) {
		...
		if ($("#uploadTailor_" + file.index).length > 0) {
			formdata.append("tailor", $("#uploadTailor_" + file.index).attr(
					"tailor"))
		}
		...
		return this
				.each(function() {
					...
					this.createHtml = function() {
						...
						var html = "";
						if (para.dragDrop) {
							html += '<form id="uploadForm" action="'
									+ para.url
							...
							var imgWidth = parseInt(para.itemWidth.replace(
									"px", "")) - 15;
							html += '<form id="uploadForm" action="'
									+ para.url
									+ '" method="post" enctype="multipart/form-data">';
							...
							html += '					<a style="height:'
									...
									+ ";width:"
									+ para.itemWidth
									+ ';" title="点击添加文件" id="rapidAddImg" class="add_imgBox" href="javascript:void(0)">';
							html += '						<div class="uploadImg" style="width:'
									+ imgWidth + 'px">';
							html += '							<img class="upload_image" src="images/add_img.png" style="width:expression(this.width > '
									+ imgWidth
									+ " ? "
									+ imgWidth
									+ 'px : this.width)" />';
					...
					this.funDisposePreviewHtml = function(file, e) {
						var html = "";
						var imgWidth = parseInt(para.itemWidth
								.replace("px", "")) - 15;
						...
						var delHtml = "";
						if (para.tailor) {
							editHtml = '<span class="file_edit" data-index="'
									+ file.index + '" title="编辑"></span>'
						}
						if (para.del) {
							delHtml = '<span class="file_del" data-index="'
									+ file.index + '" title="">x</span>'
						}
						var newStr = file.name.split("").reverse().join("");
						...
						if (file.type.indexOf("image") == 0) {
							html += '<div id="uploadList_' + file.index
									+ '" class="upload_append_list">';
							...
							html += "		</div>";
							html += "	</div>";
							html += '	<a style="height:' + para.itemHeight
									+ ";width:" + para.itemWidth
									+ ';" href="#" class="imgBox">';
							html += '		<div class="uploadImg" style="width:'
									+ imgWidth + "px;max-width:" + imgWidth
									+ "px;max-height:" + imgHeight + 'px;">';
							html += '			<img id="uploadImage_' + file.index
									+ '" class="upload_image" src="'
									+ e.target.result
									+ '" style="width:expression(this.width > '
									+ imgWidth + " ? " + imgWidth
									+ 'px : this.width);" />';
							html += "		</div>";
							html += "	</a>";
							html += '	<p id="uploadProgress_' + file.index
									+ '" class="file_progress"></p>';
							html += '	<p id="uploadFailure_' + file.index
									+ '" class="file_failure">上传失败，请重试</p>';
							html += '	<p id="uploadTailor_'
									+ file.index
									+ '" class="file_tailor" tailor="false">裁剪完成</p>';
							html += '	<p id="uploadSuccess_' + file.index
									+ '" class="file_success"></p>';
							html += "</div>"
						} else {
							html += '<div id="uploadList_' + file.index
									+ '" class="upload_append_list">';
						...
						$("body").zyPopup(
								{
									...
									onTailor : function(val, quondamImgInfo) {
										var nWidth = parseInt(para.itemWidth
												.replace("px", ""));
										var nHeight = parseInt(para.itemHeight
												.replace("px", ""));
										var qWidth = val.width;
										var qHeight = val.height;
										var ratio = nWidth / qWidth;
										var width = ratio
												* quondamImgInfo.width;
										var height = ratio
												* quondamImgInfo.height;
										var left = val.leftX * ratio;
										var top = val.rightY * ratio - qHeight
												* ratio;
					...
					this.createCorePlug = function() {
						var params = {
							...
							onSelect : function(selectFiles, allFiles) {
								...
								var funDealtPreviewHtml = function() {
									...
									if (file) {
										...
										reader.onload = function(e) {
											html += self.funDisposePreviewHtml(
													file, e);
											i++;
											funDealtPreviewHtml()
										};
								...
								//hxn20170609
								var funBindAttachEvent=function(){
									$(".upload_append_list").click(function(){
										var state=$(this).find('.file_bar');
							...
							onProgress : function(file, loaded, total) {
								var eleProgress = $("#uploadProgress_"
										+ file.index), percent = (loaded
										/ total * 100).toFixed(2)
										+ "%";
